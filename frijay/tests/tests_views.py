...
class ViewsTest(TestCase):
    ...
    def test_one_event(self):
        ...
        self.assertEqual(html.title.string, 'Frijay')
        for event in response.context:
            self.assertEqual(event['Events'][0].title, 'Shabbat')
            self.assertEqual(event['Events'][0].address, 'testaddress')
            self.assertEqual(event['Events'][0].city, 'Brooklyn')
            self.assertEqual(event['Events'][0].state, 'New York')
            self.assertEqual(event['Events'][0].phone, 1231231234)
        event_box = html.body.find(attrs={"class": "hovereffect"})
        self.assertEqual(event_box.h3.string, 'Shabbat')
        self.assertEqual(event_box.h4.string, 'Brooklyn')
        self.assertEqual(event_box.select_one("p:nth-of-type(3)").text, 'Date: Dec. 7, 2016')
        self.assertEqual(event_box.h5.text, 'Open Seats: 10')
    ...
    def test_two_events(self):
        ...
        self.assertEqual(html.title.string, 'Frijay')
        for event in response.context:
            self.assertEqual(event['Events'][0].title, 'Frijay')
            self.assertEqual(event['Events'][0].address, 'testaddress2')
            self.assertEqual(event['Events'][0].city, 'Queens')
            self.assertEqual(event['Events'][0].state, 'New York')
            self.assertEqual(event['Events'][0].phone, 1233213221)

            self.assertEqual(event['Events'][1].title, 'Shabbat')
            self.assertEqual(event['Events'][1].address, 'testaddress')
            self.assertEqual(event['Events'][1].city, 'Brooklyn')
            self.assertEqual(event['Events'][1].state, 'New York')
            self.assertEqual(event['Events'][1].phone, 1231231234)

        event_box = html.body.find_all(attrs={"class": "hovereffect"})
        self.assertEqual(event_box[0].h3.string, 'Frijay')
        self.assertEqual(event_box[0].h4.string, 'Queens')
        self.assertEqual(event_box[0].select_one("p:nth-of-type(3)").text, 'Date: Dec. 9, 2016')
        self.assertEqual(event_box[0].h5.text, 'Open Seats: 2')
        self.assertEqual(event_box[1].h3.string, 'Shabbat')
        self.assertEqual(event_box[1].h4.string, 'Brooklyn')
        self.assertEqual(event_box[1].select_one("p:nth-of-type(3)").text, 'Date: Dec. 7, 2016')
        self.assertEqual(event_box[1].h5.text, 'Open Seats: 10')
